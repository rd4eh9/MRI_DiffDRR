...
drr = DRR(
    ...
    height=352,  # Image height (if width is not provided, the generated DRR is square)
    delx=2.0,    # Pixel spacing (in mm)
).to(device)

# Set the camera pose with rotations (yaw, pitch, roll) and translations (x, y, z)
#rotations = torch.tensor([[0.0, 90.0, -105.0]], device=device)  #mri
#translations = torch.tensor([[0.0, 850.0, 0.0]], device=device) #mri
#[0.0, -90.0, -90.0]],
rotations = torch.tensor([[0.0, -120.0, -180.0]], device=device) #mammo
translations =torch.tensor([[10.0, 600.0, 20.0]], device=device) #mammo
